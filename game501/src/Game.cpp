...
static std::string srcShader = R"(
    ...
    float4 PShader(PIn pin) : SV_TARGET{ 
        ...
        // clip(color.a - 0.1f); 
        if (color.a - 0.1f < 0.0f) {
            return float4( 1.0f, 1.0f, 0.0f, 1.0f );    // Yellow, with Alpha = 1
        }
        return color; 
    } 

    float4 PShader2(PIn pin) : SV_TARGET{ 
        float4 color = SpriteTex.Sample(samLinear, pin.tex); 
        // clip(color.a - 0.1f); 
        if (color.a - 0.1f < 0.0f) {
...
void Game::CreatePixelShader() {
	...
	THROW_IF_FAILED(D3DCompile(srcShader.c_str(), srcShader.length(), nullptr,
		nullptr, nullptr, "PShader2", "ps_5_0", dwShaderFlags, 0, &shaderBlob, &errorBlob));
    THROW_IF_FAILED(m_d3dDevice->CreatePixelShader(shaderBlob->GetBufferPointer(), shaderBlob->GetBufferSize(), NULL, &m_pixelShader2));
}
...
void Game::CreateVertexBuffer() {
	...
	SimpleVertex vertices2[] = {
		{XMFLOAT3{0.5f, 1.0f, 0.4f}, {0.5f, 0.0f} } ,
		{XMFLOAT3{1.0f, 0.0f, 0.4f}, {1.0f, 1.0f} },
		{XMFLOAT3{0.0f, 0.0f, 0.4f}, {0.0f, 1.0f}},
	};
	initData.pSysMem = vertices2;
	THROW_IF_FAILED(m_d3dDevice->CreateBuffer(&bd, &initData, &m_vertexBuffer2));
}
